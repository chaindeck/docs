---
title: "Koii-ORCA Integration"
description: "Koii-ORCA Integration Guide"
---

## Objective
The purpose of this guide is to show how to integrate ORCA with Koii Network. 
Basicallly we will be focusing and simplifying so that devloper can come to this guide,
they can start devloping and running KOII-ORCA task.

## Prerequisites
1. [NodeJS>=16.0.0](https://nodejs.org/)
2. [Docker-Compose](https://docs.docker.com/compose/install/docker)
3. [ORCA](https://docs.chaindeck.io/orca/onboarding/orca-node-operator) 

## Install the KOII CLI 
To interact with the K2 locally, you need to install the Koii CLI. [Click here](https://docs.koii.network/quickstart/command-line-tool/koii-cli/install-cli) to follow the instructions. 
...
<Info>A file system wallet, aka an FS wallet, is a directory in your computer's file system. Each file in the directory holds a keypair. <br /><br /> Source [Solana](https://docs.solana.com/wallet-guide/cli#file-system-wallet)</Info>
...
<Warning>This file contains your unencrypted keypair, protect this file as it grants access to all tokens sent to its public key. Do not distribute the file; share only the public key to maintain security.</Warning>
...
You’ll be required to add your web3.storage API key, visit [here](https://web3.storage/login/) to create a web3.storage account. After creating an account, create an API Token for your project, paste the API token on this prompt
...
```file task-template/config-task.yaml
...
<Note>Make sure `nodeJS`  must be install in your system.</Note>
...
```file task-template/simple-task.js
class SimpleTask {
...
```file task-template/CoreLogic.js
...
## Run the task
Make sure you are in the `task-template` location
